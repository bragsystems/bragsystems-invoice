...
const BRAG = {
  name: "BRAG SYSTEMS",
  tagline: "India's Grain Storage Execution Partner",
...
const PrintView = forwardRef(function PrintView(props, ref) {
  const {
    docType, invoiceNo, invoiceDate, dueDate, poRef, vehicleNo, lrNo,
    modeOfTransport, incoterms, placeOfSupply, clientName, clientAddress,
    clientGstin, items, taxType, taxRate, taxable, tax, grand, notes,
  } = props;

  const s = { // inline style helpers
    border: "1px solid #ccc",
    th: { background: "#1a1a1a", color: "#fff", padding: "5px 7px", border: "1px solid #333", fontSize: "9px", fontWeight: "bold" },
    td: { border: "1px solid #ccc", padding: "5px 7px", fontSize: "10px" },
    label: { color: "#888", fontSize: "8.5px", paddingBottom: "2px" },
  };

  return (
    <div ref={ref} id="print-area" style={{ fontFamily: "Arial, sans-serif", fontSize: "11px", color: "#111", background: "#fff", padding: "14mm", minHeight: "297mm", width: "210mm", margin: "0 auto" }}>

      {/* HEADER */}
      <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "8px" }}>
        <tbody><tr>
          <td style={{ border: "none", verticalAlign: "top", width: "58%" }}>
            <div style={{ fontSize: "24px", fontWeight: "900", color: "#111", letterSpacing: "2px" }}>BRAG SYSTEMS</div>
            <div style={{ fontSize: "9px", color: "#E59215", fontWeight: "bold", letterSpacing: "0.5px", marginBottom: "5px" }}>India's Grain Storage Execution Partner</div>
          ...
          <td style={{ border: "none", verticalAlign: "top", textAlign: "right", width: "42%" }}>
            <div style={{ fontSize: "20px", fontWeight: "bold", color: "#E59215", letterSpacing: "1px" }}>{docType.toUpperCase()}</div>
            {docType === "Tax Invoice" && <div style={{ fontSize: "8px", color: "#888", marginBottom: "4px" }}>ORIGINAL FOR RECIPIENT</div>}
            <table style={{ marginLeft: "auto", borderCollapse: "collapse", fontSize: "9.5px" }}>
              <tbody>
                {[
                  ["No.", invoiceNo],
      ...
      <div style={{ borderTop: "3px solid #E59215", marginBottom: "8px" }} />

      {/* BILL TO + TRANSPORT */}
      <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "8px" }}>
        <tbody><tr>
          <td style={{ ...s.td, width: "54%", verticalAlign: "top", background: "#fafafa" }}>
            <div style={{ fontSize: "8.5px", fontWeight: "bold", color: "#E59215", marginBottom: "4px", textTransform: "uppercase", letterSpacing: "0.5px" }}>Bill To</div>
            <div style={{ fontWeight: "bold", fontSize: "12px", marginBottom: "2px" }}>{clientName || "—"}</div>
            <div style={{ fontSize: "9px", color: "#444", lineHeight: "1.6", whiteSpace: "pre-line" }}>{clientAddress}</div>
            {clientGstin && <div style={{ fontSize: "9px", marginTop: "3px" }}><strong>GSTIN:</strong> {clientGstin}</div>}
            {placeOfSupply && <div style={{ fontSize: "9px" }}><strong>Place of Supply:</strong> {placeOfSupply}</div>}
          </td>
          <td style={{ ...s.td, width: "46%", verticalAlign: "top", borderLeft: "none", background: "#fafafa" }}>
            <div style={{ fontSize: "8.5px", fontWeight: "bold", color: "#E59215", marginBottom: "4px", textTransform: "uppercase", letterSpacing: "0.5px" }}>Dispatch / Transport</div>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "8px" }}>
        <thead>
          <tr>
            {["Sr.", "Description of Goods / Services", "HSN/SAC", "Qty", "Unit", "Rate (INR)", "Amount (INR)"].map((h, i) => (
              <th key={h} style={{ ...s.th, textAlign: i === 1 ? "left" : "center", minWidth: i === 1 ? "120px" : "auto" }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {items.map((it, idx) => (
            <tr key={it.id} style={{ background: idx % 2 === 0 ? "#fff" : "#f9f9f9" }}>
              <td style={{ ...s.td, textAlign: "center", width: "28px" }}>{idx + 1}</td>
              <td style={{ ...s.td }}>{it.description}</td>
              <td style={{ ...s.td, textAlign: "center" }}>{it.hsnSac}</td>
              <td style={{ ...s.td, textAlign: "center" }}>{it.qty}</td>
              <td style={{ ...s.td, textAlign: "center" }}>{it.unit}</td>
              <td style={{ ...s.td, textAlign: "right" }}>{it.rate ? parseFloat(it.rate).toLocaleString("en-IN", { minimumFractionDigits: 2 }) : ""}</td>
              <td style={{ ...s.td, textAlign: "right", fontWeight: "bold" }}>{it.amount ? parseFloat(it.amount).toLocaleString("en-IN", { minimumFractionDigits: 2 }) : ""}</td>
            </tr>
          ))}
          {items.length < 5 && Array(5 - items.length).fill(0).map((_, i) => (
            <tr key={`e${i}`}>
              {Array(7).fill(0).map((_, j) => <td key={j} style={{ ...s.td, padding: "13px 7px" }}>&nbsp;</td>)}
            </tr>
          ))}
          <tr style={{ background: "#efefef", fontWeight: "bold" }}>
            <td colSpan={3} style={{ ...s.td, textAlign: "right", fontWeight: "bold" }}>Total</td>
            <td style={{ ...s.td, textAlign: "center", fontWeight: "bold" }}>{items.reduce((s, it) => s + (parseFloat(it.qty) || 0), 0)}</td>
            <td style={{ ...s.td }}></td>
            <td style={{ ...s.td }}></td>
            <td style={{ ...s.td, textAlign: "right", fontWeight: "bold" }}>{parseFloat(taxable).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
          </tr>
        </tbody>
      </table>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "8px" }}>
        <tbody><tr>
          <td style={{ ...s.td, width: "54%", verticalAlign: "top" }}>
            <div style={{ fontSize: "8.5px", fontWeight: "bold", color: "#E59215", marginBottom: "4px" }}>PAYMENT DETAILS</div>
            <table style={{ fontSize: "9px", borderCollapse: "collapse" }}>
              <tbody>
                {[
                  ["Bank", BRAG.bank],
                  ["Account Name", BRAG.accountName],
                  ["A/c No.", BRAG.accountNo],
                  ["IFSC", BRAG.ifsc],
                  ["UPI", BRAG.upi],
                ].map(([k, v]) => (
                  <tr key={k}>
                    <td style={{ border: "none", color: "#777", paddingRight: "12px", paddingBottom: "3px", width: "90px" }}>{k}</td>
                    <td style={{ border: "none", fontWeight: "bold" }}>{v}</td>
                  </tr>
                ))}
              </tbody>
          ...
          <td style={{ width: "46%", verticalAlign: "top", border: "none", borderLeft: "1px solid #ccc", padding: "0" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "10px" }}>
              <tbody>
                <tr>
                  <td style={{ ...s.td, border: "none", borderBottom: "1px solid #ccc", padding: "5px 8px" }}>Taxable Amount</td>
                  <td style={{ ...s.td, border: "none", borderBottom: "1px solid #ccc", textAlign: "right", padding: "5px 8px" }}>{parseFloat(taxable).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                </tr>
                {taxType === "IGST" && (
                  <tr>
                    <td style={{ ...s.td, border: "none", borderBottom: "1px solid #ccc", padding: "5px 8px" }}>IGST @ {taxRate}%</td>
                    <td style={{ ...s.td, border: "none", borderBottom: "1px solid #ccc", textAlign: "right", padding: "5px 8px" }}>{parseFloat(tax).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                  </tr>
                )}
                {taxType === "CGST+SGST" && <>
                  <tr>
                    <td style={{ ...s.td, border: "none", borderBottom: "1px solid #ccc", padding: "5px 8px" }}>CGST @ {taxRate/2}%</td>
                    <td style={{ ...s.td, border: "none", borderBottom: "1px solid #ccc", textAlign: "right", padding: "5px 8px" }}>{parseFloat(tax/2).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                  </tr>
                  <tr>
                    <td style={{ ...s.td, border: "none", borderBottom: "1px solid #ccc", padding: "5px 8px" }}>SGST @ {taxRate/2}%</td>
                    <td style={{ ...s.td, border: "none", borderBottom: "1px solid #ccc", textAlign: "right", padding: "5px 8px" }}>{parseFloat(tax/2).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                  </tr>
                </>}
                {taxType === "Nil" && (
                  <tr>
                    <td style={{ ...s.td, border: "none", borderBottom: "1px solid #ccc", padding: "5px 8px" }}>Tax (Nil/Exempt)</td>
                    <td style={{ ...s.td, border: "none", borderBottom: "1px solid #ccc", textAlign: "right", padding: "5px 8px" }}>0.00</td>
                  </tr>
                )}
                <tr style={{ background: "#1a1a1a" }}>
                  <td style={{ border: "none", padding: "7px 8px", fontWeight: "bold", fontSize: "12px", color: "#E59215" }}>GRAND TOTAL</td>
                  <td style={{ border: "none", padding: "7px 8px", textAlign: "right", fontWeight: "bold", fontSize: "12px", color: "#fff" }}>
                    INR {parseFloat(grand).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                  </td>
                </tr>
              </tbody>
            </table>
            <div style={{ padding: "5px 8px", fontSize: "8.5px", color: "#555", fontStyle: "italic", borderTop: "1px solid #ccc" }}>
              {numberToWords(grand)}
            </div>
          </td>
        </tr></tbody>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "8px" }}>
        <tbody><tr>
          <td style={{ ...s.td, width: "60%", verticalAlign: "top" }}>
            <div style={{ fontSize: "8.5px", fontWeight: "bold", color: "#E59215", marginBottom: "3px" }}>TERMS & CONDITIONS</div>
            <div style={{ fontSize: "9px", color: "#444", lineHeight: "1.7", whiteSpace: "pre-line" }}>{notes}</div>
          </td>
          <td style={{ ...s.td, width: "40%", borderLeft: "none", verticalAlign: "top", textAlign: "center" }}>
            <div style={{ fontSize: "8.5px", fontWeight: "bold", color: "#E59215", marginBottom: "4px" }}>For BRAG SYSTEMS</div>
      ...
      <div style={{ borderTop: "1px solid #E59215", paddingTop: "5px", textAlign: "center", fontSize: "8px", color: "#aaa" }}>
        Computer generated document — no signature required &nbsp;|&nbsp; {BRAG.email} &nbsp;|&nbsp; {BRAG.phone} &nbsp;|&nbsp; www.bragsystems.com
      </div>
    </div>
  );
});
...
export default function App() {
  ...
  const I = "w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:border-orange-400";
  const L = "block text-xs font-semibold text-gray-500 mb-1";

  const printProps = { docType, invoiceNo, invoiceDate, dueDate, poRef, vehicleNo, lrNo, modeOfTransport, incoterms, placeOfSupply, clientName, clientAddress, clientGstin, items, taxType, taxRate, taxable, tax, grand, notes };
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-gray-900 text-white px-5 py-2.5 flex items-center justify-between sticky top-0 z-50 shadow-lg">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={handlePrint}
            className="bg-orange-500 hover:bg-orange-600 text-white text-xs px-4 py-1.5 rounded font-bold transition-all">
            Print / PDF
          </button>
        </div>
      </div>
      ...
      {tab === "form" ? (
        <div className="max-w-3xl mx-auto p-4 space-y-4 pb-10">
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className={L}>Place of Supply (State)</label><input className={I} value={placeOfSupply} onChange={e => setPlaceOfSupply(e.target.value)} placeholder="e.g. Uttar Pradesh" /></div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className={L}>LR / e-Way Bill No.</label><input className={I} value={lrNo} onChange={e => setLrNo(e.target.value)} placeholder="LR or e-Way Bill No." /></div>
              <div>
          ...
          {/* CLIENT */}
          <div className="bg-white rounded-lg shadow p-4">
            <p className="font-bold text-gray-700 text-sm mb-3">Bill To</p>
              ...
              {items.map((it, idx) => (
                <div key={it.id} className="border border-gray-200 rounded-lg p-3 bg-gray-50">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="col-span-2"><label className={L}>Amount (auto-calculated)</label><input className={`${I} bg-orange-50 font-semibold text-orange-700`} readOnly value={it.amount ? formatINR(it.amount) : "0.00"} /></div>
                  </div>
                </div>
              ))}
        ...
        <div className="max-w-3xl mx-auto p-4">
          <p className="text-center text-xs text-gray-500 mb-3">Preview — click "Print / PDF" above to save</p>
